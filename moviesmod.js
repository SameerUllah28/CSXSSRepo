...
const BASE = "https://moviesmod.run";
...
async function search(query, cb) {
    const res = await http_get(`${BASE}/?s=${encodeURIComponent(query)}`);
    const doc = await parseHtml(res.body);
...
async function loadStreams(url, cb) {
    ...
    for (const item of parsed) {
        if (item.url.includes("gdflix")) {
            await extractGDFlix(item.url, streams);
        } else {
            streams.push(new StreamResult({
                url: item.url,
                source: `MoviesMod [${item.quality}]`,
                headers: HEADERS
            }));
        }
    }
